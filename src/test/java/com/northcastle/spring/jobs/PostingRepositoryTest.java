...
public class PostingRepositoryTest {
	...
	void getAllApplications() {
		...
		log.info("TEST.getAllApplicationsDefault(): Count = " + postingRepository.count() + "  Actual = "
				+ postings.size());
		postings.forEach(v -> {
			log.info("TEST.getAllApplicationsDefault(): " + v);
		});
	...
	void getAllPostingsDefault() {
		...
		log.info(
				"TEST.getAllPostingsDefault(): Count = " + postingRepository.count() + "  Actual = " + postings.size());
		postings.forEach(v -> {
			log.debug("TEST.getAllPostingsDefault(): " + v);
		});
		...
		// additionally, it verifies that findAll() returns all items from the database
		// properly.
		for (int index = 0; index < count; index++) {
	...
	void getAllPostingsSorted() {
		...
		log.info("TEST.getAllPostingsSorted(postingName,ASC): Count = " + postingRepository.count() + "  Actual = "
				+ postings.size());
		postings.forEach(v -> {
			log.debug("TEST.getAllPostingsSorted(postingName,ASC): " + v);
		});
		assertEquals(count, postings.size());
		// verify expected sort order
		for (int index = 0; index < count; index++) {
