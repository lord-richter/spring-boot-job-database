...
public class PostingControllerIntegrationTest {
	...
	void postEditPosting() throws Exception {
		...
		log.debug("TEST.postEditPosting(): "+mockMvc.perform(post("/postings/edit/postings/"+CommonTest.VALID_POSTING_3).with(MockMvcRequestBuilderUtils.form(existing)))
		.andExpect(status().isOk())
	...
	void postEditPosting_BadRequest() throws Exception {
		...
		log.debug("TEST.postEditPosting_Bad(): "+mockMvc.perform(post("/postings/edit/postings/"+CommonTest.VALID_POSTING_3).with(MockMvcRequestBuilderUtils.form(existing)))		
		.andExpect(status().isOk())
		.andExpect(content().string(containsString(CommonTest.FAUX_POSTING_DATA.get("postingName"))))
		.andExpect(content().string(containsString("Update Job Posting")))
		.andExpect(content().string(containsString("Pending")))
